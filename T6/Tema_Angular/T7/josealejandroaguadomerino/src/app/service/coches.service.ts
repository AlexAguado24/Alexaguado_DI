...
export class CochesService {
  ...
  getCochesFiltrados(tipoCoche: string): Coche[] {
    switch (tipoCoche) {
      case 'deportivo':
        return this.listaCoches.filter((item) => item.tipo == tipoCoche);
      case 'utilitario':
        return this.listaCoches.filter((item) => item.tipo == tipoCoche);
      default:
        return this.listaCoches;
  ...
  filtrarCochesMarcaCv(marca: string, cv: number): Coche[] {
    return this.listaCoches.filter(
      (item) => item.marca == marca && item.cv == cv
    );
  }
}
